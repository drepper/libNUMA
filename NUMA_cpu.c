...
int
NUMA_ifname_to_cpu_mask (const char *ifname, size_t destsize, cpu_set_t *dest)
{
  size_t ifname_len = strlen (ifname);
  static const char fnamefmt[] = "/sys/class/net/%s/device/local_cpus";
  char fname[ifname_len + sizeof fnamefmt - 2];
  (void) snprintf (fname, sizeof (fname), fnamefmt, ifname);
  ...
  while (1)
    {
      ssize_t n = read (fd, buf + nread, buflen - nread);
      if (n <= 0)
	{
	  close (fd);
	  if (n == 0)
  ...
  for (size_t bit = 0; nread > 0; )
    if (isxdigit (buf[--nread]))
      {
	unsigned int val = (tolower (buf[nread])
			    - (isdigit (buf[nread]) ? '0' : ('a' - 10)));
	for (size_t i = 0; i < 4; ++i, ++bit, val >>= 1)
	  if (val & 1)
	    if (CPU_SET_S (bit, destsize, dest) == 0)
	      {
		errno = ERANGE;
		return -1;
	      }
